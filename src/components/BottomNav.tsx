...
const BottomNav: React.FC = () => {
  ...
  const navItems = [
    ...
    },
    {
      path: '/ai-planner',
      icon: (active: boolean) => (
        <span className={`text-2xl mb-1 ${active ? 'scale-110' : ''} transition-all duration-200`}>🤖</span>
      ),
      label: 'AI Coach'
    },
    {
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-col items-center justify-center flex-1 rounded-2xl transition-all duration-200 mx-1 py-2 px-2 min-h-[60px] ${active
